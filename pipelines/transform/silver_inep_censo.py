#!/usr/bin/env python3
"""Silver INEP Censo Escolar — ind_matriculas_basica."""
...
INDICATOR_DEFS = [
    {
        "indicator_id": "ind_matriculas_basica",
        "name": "matriculas_educacao_basica",
        "display_name": "Matrículas — educação básica",
        "description": "Matrículas na educação básica (Censo Escolar / indicadores INEP).",
        "category": "educacao",
        "unit": "count",
        "source_id": "inep_censo",
...
def main() -> int:
    ...
    if not bronze or not (bronze / "censo_escolar_extract.jsonl").exists():
        print("SKIPPED silver_inep_censo: bronze ausente", flush=True)
        return 0
    ...
    now = utc_now()
    new_obs: list[dict] = []
    for row in read_jsonl(bronze / "censo_escolar_extract.jsonl"):
        tid = row.get("territory_id")
        year = int(row.get("ano") or 0)
        val = row.get("matriculas_basica")
        if not tid or not year or val is None:
            continue
        suf = tid.replace("uf_", "").replace("mun_", "")
        new_obs.append(
            {
                "observation_id": f"obs_mat_bas_{suf}_{year}",
                "indicator_id": "ind_matriculas_basica",
                "territory_id": tid,
                "reference_year": year,
    ...
    write_json(sil / "meta_inep_censo.json", {"em": now, "observations": len(new_obs)})
    print(f"OK silver Censo: +{len(new_obs)} obs")
    return 0
